...
object ServiceHostingKernelBuild extends Build {
  val Organization = "org.akka.essentials"
  val Version      = "0.0.1"
  val ScalaVersion = "2.10.1"
 
  lazy val ServiceHostingKernel = Project(
    id = "ServiceHosting-Kernel",
    base = file("."),
    settings = defaultSettings ++ AkkaKernelPlugin.distSettings ++ Seq(
      libraryDependencies ++= Dependencies.ServiceHostingKernel,
      distJvmOptions in Dist := "-Xms256M -Xmx256M",
      outputDirectory in Dist := file("target/ServiceHosting-dist")
    )
  )
